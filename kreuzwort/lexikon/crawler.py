...
# SSL-Warnings unterdrücken (Roche SSL-Proxy)
...
_TIMEOUT = 10
# SSL-Verifikation deaktivieren (Roche SSL-Proxy mit eigenem Zertifikat)
_VERIFY_SSL = False
...
def _crawl_kreuzwort_raetsel(frage: str, laenge: int) -> list[str]:
    ...
    try:
        ...
        url = f"https://www.kreuzwort-raetsel.net/?s={q}"
...
def _crawl_wort_suchen(frage: str, laenge: int) -> list[str]:
    ...
    try:
        ...
        # Antworten stehen in Tabellen-Zeilen: <td>WORT</td><td>5</td>
        for row in soup.find_all("tr"):
            cells = row.find_all("td")
            if len(cells) >= 2:
                wort = cells[0].get_text(strip=True).upper()
                if (wort.isalpha()
                        and len(wort) == laenge
                        and wort not in _BLACKLIST
                        and wort not in answers):
...
def crawl_answers(frage: str, laenge: int) -> list[str]:
    ...
    wort-suchen.de primär (serverseitig gerendert),
    kreuzwort-raetsel.net als Fallback (JavaScript-basiert, oft leer).

    Returns:
        Liste von Antworten in Großbuchstaben, oder leere Liste bei Fehler.
    """
    answers = _crawl_wort_suchen(frage, laenge)
    if answers:
        return answers

    return _crawl_kreuzwort_raetsel(frage, laenge)
